...
async function handleSensorSoilData(ws, message) {
...
function handleSensorMessages(ws, req, message) {
  ...
  if (isAuthorizedSensor(req) && req.headers["action"]) {
    ...
    handleSensorSoilData(ws, message);
...
module.exports = handleSensorMessages;
